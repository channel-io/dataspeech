...
SPEAKER_RATE_BINS = ["slowly", "moderately", "fast"]
...
RMS_BINS = ["speaking quietly", "speaking moderately", "speaking loudly"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rms_std_tolerance", default=3.5, type=float, help="Standard deviation tolerance for RMS estimation. Any value that is outside mean ± std * tolerance is discared. Only used if `path_to_bin_edges=False`.")
    parser.add_argument("--rms_lower_range", default=None, type=float, help="The lower range of the RMS bins")
    ...
    rms_bins = text_bins_dict.get("rms_bins", RMS_BINS)
    ...
    dataset, rms_bin_edges = bins_to_text(dataset, rms_bins, "rms", "rms", batch_size=args.batch_size, num_workers=args.cpu_num_workers, leading_split_for_bins=args.leading_split_for_bins, std_tolerance=args.rms_std_tolerance, save_dir=args.plot_directory, only_save_plot=args.only_save_plot, bin_edges=bin_edges_dict.get("rms",None), lower_range=args.rms_lower_range)
